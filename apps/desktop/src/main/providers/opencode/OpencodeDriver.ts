...
export class OpencodeDriver implements CliDriver {
  ...
  private trackTurn(params: {
    ...
    const pollTimer = setInterval(() => {
      void this.pollSessionState(params.turnId);
    }, 2000);
    pollTimer.unref?.();
    this.pollTimers.set(params.turnId, pollTimer);
  }
  ...
  private async pollSessionState(turnId: string): Promise<void> {
    ...
    try {
      ...
      if (res.ok && !((res.headers.get("content-type") ?? "").includes("text/html"))) {
        ...
        if (seen) {
          for (const event of diffLiveTools(seen, payload, turnId)) this.emit(event);
        }
        const meta = this.turnMeta.get(turnId);
